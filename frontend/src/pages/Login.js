...
export default function Login() {
  ...
  const toggle = () => {
    setError("");
    if (mode === "login") { setEmail(""); setPassword(""); }
    ...
    <div className="min-h-screen grid lg:grid-cols-2">
      ...
      <div className="flex items-center justify-center p-6 md:p-12 bg-background">
        <div className="w-full max-w-sm animate-fade-in">
          ...
          <h1 className="font-heading text-3xl font-bold tracking-tight">{mode === "login" ? "Sign in" : "Create account"}</h1>
          <p className="text-muted-foreground mt-2 mb-8">{mode === "login" ? "Enter your credentials to continue." : "Set up your own mill account."}</p>

          <form onSubmit={submit} className="space-y-5">
            {mode === "register" && (
              <div className="space-y-2">
                <Label htmlFor="name">Full name</Label>
                <Input id="name" data-testid="register-name" value={name} onChange={(e) => setName(e.target.value)} className="h-12" required />
              </div>
            )}
            <div className="space-y-2">
            ...
            {mode === "register" && (
              <div className="space-y-2">
                ...
                <Select value={role} onValueChange={setRole}>
                  <SelectTrigger className="h-12" data-testid="register-role"><SelectValue /></SelectTrigger>
                  <SelectContent>
            ...
            <Button data-testid={mode === "login" ? "login-submit" : "register-submit"} type="submit" disabled={loading} className="w-full h-12 text-base active:scale-95 transition-transform">
              {loading && <Loader2 className="h-4 w-4 mr-2 animate-spin" />} {mode === "login" ? "Sign in" : "Create account"}
            </Button>
          </form>

          <p className="mt-6 text-sm text-muted-foreground text-center">
            {mode === "login" ? "New to the mill? " : "Already have an account? "}
            <button type="button" onClick={toggle} className="text-primary font-semibold hover:underline" data-testid="toggle-auth-mode">
