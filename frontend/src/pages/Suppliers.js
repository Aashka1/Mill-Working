          ...
          <TableBody>
            {filtered.map((s) => (
              <TableRow key={s.id} className="hover:bg-muted/50" data-testid={`supplier-row-${s.id}`}>
                ...
                <TableCell className="text-right"><Button variant="ghost" size="icon" onClick={() => suppliers.remove(s.id)}><Trash2 className="h-4 w-4 text-destructive" /></Button></TableCell>
              </TableRow>
            ))}
